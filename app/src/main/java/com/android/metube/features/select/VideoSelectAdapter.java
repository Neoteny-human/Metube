...
import com.android.metube.features.trim.VideoTrimmerActivity;
...
public class VideoSelectAdapter extends CursorAdapter {
    ...
    @Override public void bindView(View view, Context context, final Cursor cursor) {
        ...
        @SuppressLint("Range") final String path = cursor.getString(cursor.getColumnIndex(MediaStore.Video.Media.DATA));
    ...
    private Uri getVideoUri(Cursor cursor) {
        @SuppressLint("Range") String id = cursor.getString(cursor.getColumnIndex(MediaStore.MediaColumns._ID));
